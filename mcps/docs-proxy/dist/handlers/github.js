import { processFetchedText } from "./url.js";
export function isGitHubRef(query) {
    return (/^https?:\/\/github\.com\/[\w.-]+\/[\w.-]+/i.test(query) ||
        /^[\w.-]+\/[\w.-]+(\s|$)/.test(query));
}
export function parseGitHubRef(query) {
...
export async function fetchFromGitHub(ownerRepo, topic) {
