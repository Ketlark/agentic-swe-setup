...
assumption: Models blend code-style and spec-conformance feedback into one undifferentiated list, making findings hard to triage and act on. Re-evaluate when models spontaneously separate review axes without explicit instruction.
triggers: review, check changes, PR, pull request, diff, since branch, since commit, standards check
outputs: Standards report (per-file findings citing the standard), Spec report (missing/extra/wrong citing spec line), one-line summary per axis
budget: each sub-agent max 20 tool calls, under 400 words output
done_when: both reports presented under separate headings with summary line; spec axis noted if unavailable
---
...
**required_outputs**: Standards report (per-file findings, citing the standard), Spec report (missing/extra/wrong, citing the spec line), one-line summary with total findings per axis.

**budget**: Each sub-agent: max 20 tool calls, under 400 words output.

**done_when**: Both sub-agent reports are presented under separate headings with the summary line. No axis is missing (unless spec is unavailable, which must be noted).
...
1. Issue references in the commit messages (`#123`, `Closes #45`, GitLab `!67`, etc.) — fetch via the project's issue tracker (`gh issue view`, GitLab API, etc.).
...
Spawn the two sub-agents in parallel using whatever subagent mechanism the host harness exposes (Pi sets `PI_SUBAGENT_DEPTH`; bash-guard hard-blocks catastrophic ops in subagent mode, so reviewer subagents are safe to run).
